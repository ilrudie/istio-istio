...
func Equal[O any](a, b O) bool {
	...
	// EqualerProvider is probed after Equaler so types implementing Equals pay nothing new here;
	// resolveEquals probes it first instead, where the resolution happens only once.
	if pk, ok := any(a).(EqualerProvider[O]); ok {
		return pk.EqualsFunc()(a, b)
	}
	if pk, ok := any(&a).(EqualerProvider[O]); ok {
		return pk.EqualsFunc()(a, b)
	}
...
// Equal's per-call probing (and its boxing allocations) on every comparison. EqualerProvider is
// probed first since its function avoids boxing entirely; the remaining probes mirror Equal's,
// in the same order. A type can have only one Equals method (and EqualsFunc must be consistent
// with it), so the resolved function always compares the same way Equal would.
func resolveEquals[O any]() func(a, b O) bool {
	...
	var zero O
	// Tier zero: the type hands over its comparison function directly, so every comparison is a
	// plain function call with no interface boxing.
	if p, ok := any(zero).(EqualerProvider[O]); ok {
		return providedEquals[O](p)
	}
	if p, ok := any(&zero).(EqualerProvider[O]); ok {
		return providedEquals[O](p)
	}
...
// providedEquals extracts the comparison function from an EqualerProvider, panicking with a
// useful message if the implementation returns nil (which would otherwise surface as an opaque
// nil-function panic at first comparison).
func providedEquals[O any](p EqualerProvider[O]) func(a, b O) bool {
	fn := p.EqualsFunc()
	if fn == nil {
